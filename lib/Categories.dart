...
class _CategoriesPageState extends State<CategoriesPage> {
  ...
  final String basicAuth = 'Basic ' +
      base64.encode(utf8.encode('HXK91J3162VDCQR8DAZD7Y77PT1Z76WD:'));
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var category in categories) {
          final int id = (category['id'] is String)
  ...
  Future<Map<String, dynamic>> fetchCategoryInfo(int id) async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://localhost/presta/api/categories/$id?output_format=JSON'),
        headers: <String, String>{'authorization': basicAuth},
      );
  ...
  Future<String> fetchProductImageForCategory(
      int productId, int imageId) async {
    try {
      final imageUrl =
          'http://localhost/presta/api/images/products/$productId/$imageId?ws_key=HXK91J3162VDCQR8DAZD7Y77PT1Z76WD';
      return imageUrl;
    } catch (e) {
      print('Une erreur s\'est produite : $e');
    }
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(
                      width:
                          10), // Ajustez l'espacement horizontal si nécessaire
                  Expanded(
                    child: Center(
                      child: Text(
                        'Nos catégories',
                ...
                builder: (context, snapshot) {
                  ...
                  return GridView.builder(
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                      ...
                      childAspectRatio:
                          0.75, // Réglez cette valeur selon vos besoins
                    ),
                    itemCount: categories.length,
                    itemBuilder: (context, index) {
                      final productId = categories[index]['id'];
                      final imageId = categories[index]['id_default_image'] ??
                          productId; // Remplacez 0 par une valeur par défaut si nécessaire
                      final imageUrl =
                          fetchProductImageForCategory(productId, imageId);
                                    ...
                                    Padding(
                                      ...
                                      child: Center(
                                        child: Text(
                                          ...
                                          style: TextStyle(
                                            fontSize:
                                                16, // Vous pouvez ajuster la taille de la police selon vos préférences
